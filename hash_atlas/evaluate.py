...
from loss_utils import (
    get_rigidity_loss,
    get_optical_flow_loss_all,
    get_optical_flow_alpha_loss_all,
)
...
def interpolate_alpha(m1_alpha_v, m1_alpha_u, m1_alpha_alpha):
    ...
    masks1 = griddata(
        (m1_alpha_u, m1_alpha_v), m1_alpha_alpha, (xv, yv), method="linear"
    )
    return masks1
...
def get_colors(resolution, minx, maxx, miny, maxy, pointx, pointy, image):
    ...
    mx_logicaly = np.logical_and(
        np.ceil(pointy2) < resolution, np.floor(pointy2) < resolution
    )
    mx_logicaxlx = np.logical_and(
        np.ceil(pointx2) < resolution, np.floor(pointx2) < resolution
    )
...
def get_high_res_texture(
    ...
    with torch.no_grad():
        ...
        for i in indsy:
            ...
            input_ori = torch.cat(
                (indsx.unsqueeze(1), i * torch.ones_like(indsx.unsqueeze(1))), dim=1
            ).to(device)
            ...
            reconstruction_texture2[counter, :, :] = (
                model_F_atlas(input_rearrange).detach().cpu()
            )
...
def get_mapping_area(
    ...
    with torch.no_grad():
        for i in range(len(relisa)):
            ...
            relfs = (
                torch.from_numpy(relfsa[i]).unsqueeze(1) / (number_of_frames / 2) - 1
            )
            mapping_outs = model_F_mapping(
                torch.cat((reljs, relis, relfs), dim=1).to(device)
            ).cpu()
            if is_foreground:
                uv = mapping_outs[..., :2]
            else:
...
def normalize_uv_images(uv_frames_reconstruction, values_shift, edge_size, minx, miny):
    uv_frames_reconstruction[:, :, 0, :] = (
        (uv_frames_reconstruction[:, :, 0, :] * 0.5 + values_shift) - np.float64(minx)
    ) / edge_size
    uv_frames_reconstruction[:, :, 1, :] = (
        (uv_frames_reconstruction[:, :, 1, :] * 0.5 + values_shift) - np.float64(miny)
    ) / edge_size
...
def evaluate_model(
    ...
    checkerboard_ = (
        np.array(Image.open(os.path.join(current_dir, "checkerboard.png"))).astype(
            np.float64
        )
        / 255.0
    )
    ...
    video_frames_reconstruction_edit1_checkerboard = (
        np.ones((resy, resx, 3, number_of_frames)) * 0.5
    )
    ...
    with torch.no_grad():
        for f in range(number_of_frames):
            ...
            for i in range(len(relisa)):
                ...
                try:
                    masks1[
                        np.ceil(pointsy1).astype((np.int64)),
                    ...
                    masks1[
                        np.floor(pointsy1).astype((np.int64)),
                ...
                try:
                    masks2[
                        np.ceil(pointsy2).astype((np.int64)),
                        np.ceil(pointsx2).astype((np.int64)),
                    ] = 1
                    masks2[
                        np.floor(pointsy2).astype((np.int64)),
                        np.floor(pointsx2).astype((np.int64)),
                    ] = 1
                    masks2[
                        np.floor(pointsy2).astype((np.int64)),
                        np.ceil(pointsx2).astype((np.int64)),
                ...
                video_frames_reconstruction_edit1[
                    relisa[i][relevant1], reljsa[i][relevant1], :, f
                ] = (rgb21 * alpha.cpu().numpy()[relevant1])
                video_frames_reconstruction_edit2[
                    relisa[i][relevant2], reljsa[i][relevant2], :, f
                ] = rgb22
                video_frames_reconstruction_edit[
                    relisa[i][relevant1], reljsa[i][relevant1], :, f
                ] = (
                    video_frames_reconstruction_edit[
                        relisa[i][relevant1], reljsa[i][relevant1], :, f
                ...
                video_frames_reconstruction_edit[
                    ...
                    + rgb22 * (1 - alpha).cpu().numpy()[relevant2]
                )

                video_frames_reconstruction[relisa[i], reljsa[i], :, f] = (
                    rgb_current.detach().cpu().numpy()
                )
                alpha_reconstruction[relisa[i], reljsa[i], f] = (
                    alpha[:, 0].detach().cpu().numpy()
                )
                flow_loss1_video[relisa[i], reljsa[i], f] = flow_loss1.cpu().numpy()
                flow_loss2_video[relisa[i], reljsa[i], f] = flow_loss2.cpu().numpy()
                flow_alpha_loss_video[relisa[i], reljsa[i], f] = (
                    flow_alpha_loss.cpu().squeeze().numpy()
                )
                rigidity_loss1_video[relisa[i], reljsa[i], f] = (
                    rigidity_loss1.cpu().numpy()
                )
                rigidity_loss2_video[relisa[i], reljsa[i], f] = (
                    rigidity_loss2.cpu().numpy()
                )
                rgb_error_video[relisa[i], reljsa[i], f] = (
                    (video_frames[relisa[i], reljsa[i], :, f] - rgb_current.cpu()).norm(
                        dim=1
                    )
                    ** 2
                ).numpy()
                rgb_residual_video[relisa[i], reljsa[i], :, f] = (
                    (video_frames[relisa[i], reljsa[i], :, f] - rgb_current.cpu())
                ).numpy()
                video_frames_reconstruction_edit1_checkerboard[
                    relisa[i][relevant1_tex], reljsa[i][relevant1_tex], :, f
                ] = (
                    video_frames_reconstruction_edit1_checkerboard[
                        relisa[i][relevant1_tex], reljsa[i][relevant1_tex], :, f
                    ]
                    * (1 - alpha).cpu().numpy()[relevant1_tex]
                    + rgb21_tex * alpha.cpu().numpy()[relevant1_tex]
                )

                video_frames_reconstruction_edit2_checkerboard[
                    relisa[i][relevant2_tex], reljsa[i][relevant2_tex], :, f
                ] = rgb22_tex
    ...
    if show_atlas_alpha:  # this part is slow, not running during training
        ...
        mpimg.imsave(
            ...
            (
                np.concatenate(
                    (texture_orig1.numpy(), masks1_alpha[:, :, np.newaxis]), axis=2
                )
                * (255)
            ).astype(np.uint8),
        )
        mpimg.imsave(
            ...
            (
                np.concatenate(
                    (texture_orig2.numpy(), masks2_alpha[:, :, np.newaxis]), axis=2
                )
                * (255)
            ).astype(np.uint8),
        )

    uv1_frames_reconstruction = normalize_uv_images(
        uv1_frames_reconstruction, 0.5, edge_size, minx, miny
    )
    uv2_frames_reconstruction = normalize_uv_images(
        uv2_frames_reconstruction, -0.5, edge_size2, minx2, miny2
    )

    Path(evaluation_folder).mkdir(parents=True, exist_ok=True)
    mpimg.imsave(
        "%s/atlas_edited_word_foreground.png" % (evaluation_folder),
        (masks1[:, :, np.newaxis] * edited_tex1.numpy() * (255)).astype(np.uint8),
    )
    mpimg.imsave(
        "%s/atlas_edited_word_background.png" % (evaluation_folder),
        (masks2[:, :, np.newaxis] * edited_tex2.numpy() * (255)).astype(np.uint8),
    )

    mpimg.imsave(
        "%s/atlas_ori_foreground.png" % (evaluation_folder),
        (masks1[:, :, np.newaxis] * texture_orig1.numpy() * (255)).astype(np.uint8),
    )
    mpimg.imsave(
        "%s/atlas_ori_background.png" % (evaluation_folder),
        (masks2[:, :, np.newaxis] * texture_orig2.numpy() * (255)).astype(np.uint8),
    )

    mpimg.imsave(
        "%s/atlas_ori_foreground.png" % (results_folder.parent.parent / "final_atlas"),
        (masks1[:, :, np.newaxis] * texture_orig1.numpy() * (255)).astype(np.uint8),
    )
    mpimg.imsave(
        "%s/atlas_ori_background.png" % (results_folder.parent.parent / "final_atlas"),
        (masks2[:, :, np.newaxis] * texture_orig2.numpy() * (255)).astype(np.uint8),
    )

    writer_t_edited1 = imageio.get_writer(
        "%s/add_word_to_foreground.mp4" % (evaluation_folder), fps=10
    )
    writer_t_edited2 = imageio.get_writer(
        "%s/add_word_to_background.mp4" % (evaluation_folder), fps=10
    )

    writer_t_edited1_tex = imageio.get_writer(
        "%s/add_checkboard_to_foreground.mp4" % (evaluation_folder), fps=10
    )
    writer_t_edited2_tex = imageio.get_writer(
        "%s/add_checkboard_to_background.mp4" % (evaluation_folder), fps=10
    )

    writer_alpha = imageio.get_writer(
        "%s/alpha_visualization.mp4" % (evaluation_folder), fps=10
    )
    writer_alpha_vs_mask = imageio.get_writer(
        "%s/alpha_vs_mask.mp4" % (evaluation_folder), fps=10
    )
    ...
    writer_edit = imageio.get_writer(
        "%s/reconstruction_from_edited_atlas_word.mp4" % (evaluation_folder), fps=10
    )
    writer_checkerboard_1 = imageio.get_writer(
        "%s/reconstruction_from_edited_atlas_checkboard_foreground.mp4"
        % (evaluation_folder),
        fps=10,
    )
    writer_checkerboard_2 = imageio.get_writer(
        "%s/reconstruction_from_edited_atlas_checkboard_background.mp4"
        % (evaluation_folder),
        fps=10,
    )
    ...
    writer_uv_1 = imageio.get_writer(
        "%s/uvMap_foreground.mp4" % (evaluation_folder), fps=10
    )
    writer_uv_2 = imageio.get_writer(
        "%s/uvMap_background.mp4" % (evaluation_folder), fps=10
    )
    writer_uv_1_masked = imageio.get_writer(
        "%s/uvMap_foreground_masked.mp4" % (evaluation_folder), fps=10
    )

    writer_global_info = imageio.get_writer(
        "%s/loss_visualization.mp4" % (evaluation_folder), fps=10
    )
    ...
    for i in range(number_of_frames):
        ...
        alpha_vs_mask_cur = np.transpose(
            np.stack(
                (
                    mask_frames[:, :, i].numpy(),
                    alpha_reconstruction[:, :, i],
                    np.zeros_like(mask_frames[:, :, i].numpy()),
                )
            ),
            (1, 2, 0),
        )
        ...
        writer_edit.append_data(
            (video_frames_reconstruction_edit[:, :, :, i] * (255)).astype(np.uint8)
        )
        ...
        writer_uv_1_masked.append_data(
            (
                ...
                * alpha_reconstruction[:, :, i][:, :, np.newaxis]
                * (255)
            ).astype(np.uint8)
        )
        ...
        ssim[i] = compute_ssim(
            video_frames[..., i],
            torch.from_numpy(video_frames_reconstruction[..., i]),
            1.0,
        )
        lpips[i] = compute_LPIPS(
            video_frames[None, ..., i]
            .type(torch.float32)
            .permute([0, 3, 1, 2])
            .contiguous()
            .cuda(),
            torch.from_numpy(video_frames_reconstruction[None, ..., i])
            .type(torch.float32)
            .permute([0, 3, 1, 2])
            .contiguous()
            .cuda(),
    ...
    file1 = open(
        "%s/%06d/PSNR_%f_ssim_%f_lpips_%f"
        % (results_folder, iteration, psnrs.mean(), ssim.mean(), lpips.mean()),
        "a",
    )
    file1.close()
    if save_checkpoint:
