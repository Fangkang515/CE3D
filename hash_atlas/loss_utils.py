...
def get_gradient_loss(video_frames, video_frames_dx, video_frames_dy, jif_current,
    ...
    rgb_output1_xplus1yt = (model_F_atlas(uv_foreground1_xplus1yt * 0.25 + 0.75) + 1.0) * 0.5
    rgb_output2_xplus1yt = (model_F_atlas(uv_foreground2_xplus1yt * 0.25 + 0.25) + 1.0) * 0.5
    # rgb_output1_xplus1yt, rgb_output2_xplus1yt = merge_uv_to_atlas(uv_foreground1_xplus1yt, uv_foreground2_xplus1yt)

    rgb_output1_xyplus1t = (model_F_atlas(uv_foreground1_xyplus1t * 0.25 + 0.75) + 1.0) * 0.5
    rgb_output2_xyplus1t = (model_F_atlas(uv_foreground2_xyplus1t * 0.25 + 0.25) + 1.0) * 0.5
    ...
    rgb_output_pred_xyplus1t = rgb_output1_xyplus1t * alphayplus1 + rgb_output2_xyplus1t * (1.0 - alphayplus1)
    rgb_output_pred_xplus1yt = rgb_output1_xplus1yt * alphaxplus1 + rgb_output2_xplus1yt * (1.0 - alphaxplus1)
...
def get_rigidity_loss(jif_foreground, derivative_amount, resx, number_of_frames, model_F_mapping, uv_foreground, device,
                      uv_mapping_scale=1.0, return_all=False, is_foreground=True):
...
def get_optical_flow_loss_all(jif_foreground, uv_foreground,
    ...
    if is_foreground:
        uv_foreground_forward_should_match = model_F_mapping(xyt_foreground_forward_should_match.to(device))[..., :2]
    else:
        uv_foreground_forward_should_match = model_F_mapping(xyt_foreground_forward_should_match.to(device))[..., 2:4]
...
def get_optical_flow_loss(jif_foreground, uv_foreground, optical_flows_reverse, optical_flows_reverse_mask, resx,
                          ...
                          device, use_alpha=False, alpha=1.0, is_foreground=True):
    # Forward flow:
    ...
    if is_foreground:
        uv_foreground_backward_should_match = model_F_mapping(xyt_foreground_backward_should_match.to(device))[..., :2]
    else:
        uv_foreground_backward_should_match = model_F_mapping(xyt_foreground_backward_should_match.to(device))[..., 2:4]
    loss_flow_prev = (uv_foreground_backward_should_match - uv_foreground_backward_relevant).norm(dim=1) * resx / (2 * uv_mapping_scale)
...
def get_optical_flow_alpha_loss(model_F_mapping,
    ...
    alpha_foreground_forward_should_match = model_F_mapping(xyt_foreground_forward_should_match.to(device))[..., -1:]
    ...
    xyt_foreground_backward_should_match, relevant_batch_indices_backward = get_corresponding_flow_matches(
        ...
        use_uv=False)
    alpha_foreground_backward_should_match = model_F_mapping(xyt_foreground_backward_should_match.to(device))[..., -1:]
...
def get_optical_flow_alpha_loss_all(model_F_mapping,
