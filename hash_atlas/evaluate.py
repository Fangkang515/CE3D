...
from metric_utils import compute_ssim, init_lpips
...
def get_high_res_texture(resolution, minx, maxx, miny, maxy, model_F_atlas, device, rearrange_type='foreground'):
    ...
    with torch.no_grad():
        ...
        for i in indsy:
            ...
            input_ori = torch.cat((indsx.unsqueeze(1), i * torch.ones_like(indsx.unsqueeze(1))), dim=1).to(device)
            if rearrange_type == 'foreground':  # ([0, 1]+1)*0.5 --> [0.5, 1]
                input_rearrange = (input_ori + 1) * 0.5
            elif rearrange_type == 'background':  # ([-1, 0]+1)*0.5-->[0, 0.5]
                input_rearrange = (input_ori + 1) * 0.5
            else:
                raise ValueError(rearrange_type)
            reconstruction_texture2[counter, :, :] = model_F_atlas(input_rearrange).detach().cpu()
...
# Given a mapping model "model_F_mapping", masks "mask_frames" and alpha network "F_alpha", find
# a range in the uv domain, that mask points are mapped to by model_F_mapping.
def get_mapping_area(model_F_mapping, mask_frames, resx, number_of_frames, uv_shift, device,
                     invert_alpha=False,
                     alpha_thresh=-0.5,
                     is_foreground=True):
    ...
    with torch.no_grad():
        for i in range(len(relisa)):
            ...
            mapping_outs = model_F_mapping(torch.cat((reljs, relis, relfs), dim=1).to(device)).cpu()
            if is_foreground:
                uv = mapping_outs[..., :2]
            else:
                uv = mapping_outs[..., 2:4]
            # XXX alpha may should be rearrange to [0,1]
            alpha = mapping_outs[..., -1:].squeeze()
            if invert_alpha:
                alpha = -alpha
    ...
    maxx = np.minimum(maxx, 1)  # if  torch.any(alpha > alpha_thresh)=False, value=-1
    maxy = np.minimum(maxy, 1)  # if ... value=-1

    minx = np.maximum(minx, -1)  # if ... value=1
    miny = np.maximum(miny, -1)  # if ... value=1
...
def evaluate_model(model_F_atlas, resx, resy, number_of_frames, model_F_mapping,
                   video_frames,
                   results_folder, iteration, mask_frames, optimizer_all, writer, derivative_amount,
                   uv_mapping_scale, optical_flows, optical_flows_mask, device,
                   save_checkpoint=True, show_atlas_alpha=False):  #
    # results_folder like: /data/nerf_llff_data/flower/work_space/atlas_space/training_results
    ...
    if save_checkpoint:
        torch.save({
            'model_F_mapping_state_dict': model_F_mapping.state_dict(),
        ...
        torch.save({
            ...
            'iteration': iteration,
            'optimizer_all_state_dict': optimizer_all.state_dict()
        }, '%s/checkpoint' % (results_folder.parent.parent / 'final_atlas'))

    # get relevant bounding box in the foreground atlas coordinates, using the input masks.
    minx = 0
    miny = 0
    edge_size = 1
    # below min max... only used for checkerboard
    maxxt, minxt, maxyt, minyt, edge_sizet = get_mapping_area(model_F_mapping, mask_frames > 0.5,
                                                              larger_dim,
                                                              number_of_frames,
                                                              torch.tensor([0.5, 0.5]), device, invert_alpha=False,
                                                              alpha_thresh=0.95, is_foreground=True)
    maxx2, minx2, maxy2, miny2, edge_size2 = get_mapping_area(model_F_mapping, mask_frames <= 0.5,
                                                              larger_dim,
                                                              number_of_frames,
                                                              torch.tensor([-0.5, -0.5]), device,
                                                              invert_alpha=True, is_foreground=False)
    print('--->mapping_area foreground: ', maxxt.item(), minxt.item(), maxyt.item(), minyt.item(), edge_sizet.item())
    print('--->mapping_area background: ', maxx2.item(), minx2.item(), maxy2.item(), miny2.item(), edge_size2.item())
    # edited_tex1: 前景加字后的edit atlas图; texture_orig1: 前景的原atlas图 
    # XXX 暂时不知为什么这里的minx、edge_size等是直接指定，而求解edit_tex2的minx2却是经过上面的步骤计算
    edited_tex1, texture_orig1 = get_high_res_texture(
        1000,
        minx, minx + edge_size, miny, miny + edge_size, model_F_atlas, device, 'foreground'
    )

    # edited_tex2: 背景加字后的edit atlas图; texture_orig2: 背景的原atlas图 
    edited_tex2, texture_orig2 = get_high_res_texture(
        1000,
        minx2, minx2 + edge_size2, miny2, miny2 + edge_size2, model_F_atlas, device, 'background'
    )
    '''
    # 实际上该代码替换上面也可以。上面只不过把范围缩小了一点点
    edited_tex2, texture_orig2 = get_high_res_texture(
        1000,
        -1, 0, -1, 0, model_F_atlas, device, 'background'
    )
    '''

    # only used for checkerboard generatation
    _, texture_orig1t = get_high_res_texture(
        500,
        minxt, minxt + edge_sizet, minyt, minyt + edge_sizet, model_F_atlas, device, 'foreground'
    )
    ...
    checkerboard_ = np.array(Image.open(os.path.join(current_dir, "checkerboard.png"))).astype(np.float64) / 255.
    ...
    def merge_uv_to_atlas(uv_foreground1, uv_foreground2):
        ...
        uv_datas = torch.cat([uv_foreground1 * 0.25 + 0.75, uv_foreground2 * 0.25 + 0.25], dim=0)
        rgb_outputs = (model_F_atlas(uv_datas) + 1) * 0.5
        rgb_output1, rgb_output2 = rgb_outputs[:split_idx, ...], rgb_outputs[split_idx:, ...]
    ...
    with torch.no_grad():
        for f in range(number_of_frames):
            ...
            for i in range(len(relisa)):
                ...
                mapping_outs = model_F_mapping(torch.cat((reljs, relis, (f / (number_of_frames / 2.0) - 1) * torch.ones_like(relis)), dim=1).to(device))
                uv_temp1 = mapping_outs[..., :2]
                uv_temp2 = mapping_outs[..., 2:4]
                alpha = mapping_outs[..., -1:]
                # Sample RGB values from the atlas:
                rgb_current1 = model_F_atlas(uv_temp1 * 0.25 + 0.75)
                rgb_current2 = model_F_atlas(uv_temp2 * 0.25 + 0.25)
                rgb_current1 = (rgb_current1 + 1) * 0.5
                rgb_current2 = (rgb_current2 + 1) * 0.5
                # rgb_current1, rgb_current2 = merge_uv_to_atlas(uv_temp1, uv_temp2)
                ...
                rigidity_loss1 = get_rigidity_loss(
                    ...
                    model_F_mapping,
                    uv_temp1, device,
                    uv_mapping_scale=uv_mapping_scale, return_all=True, is_foreground=True)
                rigidity_loss2 = get_rigidity_loss(
                    ...
                    model_F_mapping,
                    uv_temp2, device,
                    uv_mapping_scale=uv_mapping_scale, return_all=True, is_foreground=False)
                ...
                if f < (number_of_frames - 1):
                    flow_loss1 = get_optical_flow_loss_all(
                        ...
                        number_of_frames, model_F_mapping, optical_flows, optical_flows_mask, uv_mapping_scale, device,
                        alpha=alpha, is_foreground=True)

                    flow_loss2 = get_optical_flow_loss_all(
                        jif_foreground, uv_temp2, larger_dim,
                        number_of_frames, model_F_mapping, optical_flows, optical_flows_mask, uv_mapping_scale, device,
                        alpha=1 - alpha, is_foreground=False)
                ...
                flow_alpha_loss = get_optical_flow_alpha_loss_all(model_F_mapping,
    ...
    mpimg.imsave("%s/atlas_edited_word_foreground.png" % (evaluation_folder), (masks1[:, :, np.newaxis] * edited_tex1.numpy() * (255)).astype(np.uint8))
    mpimg.imsave("%s/atlas_edited_word_background.png" % (evaluation_folder), (masks2[:, :, np.newaxis] * edited_tex2.numpy() * (255)).astype(np.uint8))

    mpimg.imsave("%s/atlas_ori_foreground.png" % (evaluation_folder), (masks1[:, :, np.newaxis] * texture_orig1.numpy() * (255)).astype(np.uint8))
    mpimg.imsave("%s/atlas_ori_background.png" % (evaluation_folder), (masks2[:, :, np.newaxis] * texture_orig2.numpy() * (255)).astype(np.uint8))

    mpimg.imsave("%s/atlas_ori_foreground.png" % (results_folder.parent.parent / 'final_atlas'), (masks1[:, :, np.newaxis] * texture_orig1.numpy() * (255)).astype(np.uint8))
    mpimg.imsave("%s/atlas_ori_background.png" % (results_folder.parent.parent / 'final_atlas'), (masks2[:, :, np.newaxis] * texture_orig2.numpy() * (255)).astype(np.uint8))

    writer_t_edited1 = imageio.get_writer("%s/add_word_to_foreground.mp4" % (evaluation_folder), fps=10)
    writer_t_edited2 = imageio.get_writer("%s/add_word_to_background.mp4" % (evaluation_folder), fps=10)

    writer_t_edited1_tex = imageio.get_writer("%s/add_checkboard_to_foreground.mp4" % (evaluation_folder), fps=10)
    writer_t_edited2_tex = imageio.get_writer("%s/add_checkboard_to_background.mp4" % (evaluation_folder), fps=10)

    writer_alpha = imageio.get_writer("%s/alpha_visualization.mp4" % (evaluation_folder), fps=10)
    writer_alpha_vs_mask = imageio.get_writer("%s/alpha_vs_mask.mp4" % (evaluation_folder), fps=10)

    writer_im_rec = imageio.get_writer("%s/reconstruction_from_atlas.mp4" % (evaluation_folder), fps=10)
    writer_residuals = imageio.get_writer("%s/reconstruction_error_visualization.mp4" % (evaluation_folder), fps=10)
    writer_edit = imageio.get_writer("%s/reconstruction_from_edited_atlas_word.mp4" % (evaluation_folder), fps=10)
    writer_checkerboard_1 = imageio.get_writer("%s/reconstruction_from_edited_atlas_checkboard_foreground.mp4" % (evaluation_folder), fps=10)
    writer_checkerboard_2 = imageio.get_writer("%s/reconstruction_from_edited_atlas_checkboard_background.mp4" % (evaluation_folder), fps=10)

    # XXX 
    writer_uv_1 = imageio.get_writer("%s/uvMap_foreground.mp4" % (evaluation_folder), fps=10)
    writer_uv_2 = imageio.get_writer("%s/uvMap_background.mp4" % (evaluation_folder), fps=10)
    writer_uv_1_masked = imageio.get_writer("%s/uvMap_foreground_masked.mp4" % (evaluation_folder), fps=10)

    writer_global_info = imageio.get_writer("%s/loss_visualization.mp4" % (evaluation_folder), fps=10)
    psnrs = np.zeros((number_of_frames, 1))
    lpips = np.zeros((number_of_frames, 1))
    ssim = np.zeros((number_of_frames, 1))
    ...
    for i in range(number_of_frames):
        ...
        alpha_vs_mask_cur = np.transpose(np.stack(
            (mask_frames[:, :, i].numpy(), alpha_reconstruction[:, :, i], np.zeros_like(mask_frames[:, :, i].numpy()))),
                                              (1, 2, 0))

        writer_alpha_vs_mask.append_data((alpha_vs_mask_cur * 255.0).astype(np.uint8))
        ...
        psnrs[i] = skimage.metrics.peak_signal_noise_ratio(
            video_frames[:, :, :, i].numpy(),
            video_frames_reconstruction[:, :, :, i],
            data_range=1)
        
        ssim[i] = compute_ssim(video_frames[..., i], torch.from_numpy(video_frames_reconstruction[..., i]), 1.)
        lpips[i] = compute_LPIPS(video_frames[None, ..., i].type(torch.float32).permute([0, 3, 1, 2]).contiguous().cuda(), torch.from_numpy(video_frames_reconstruction[None, ..., i]).type(torch.float32).permute([0, 3, 1, 2]).contiguous().cuda(), normalize=True).item()
        print(i, psnrs[i], ssim[i], lpips[i])
        ...
        plt.title("alpha")
        plt.subplot(3, 4, 6)
        plt.imshow(alpha_vs_mask_cur, vmin=0.0, vmax=1.0)
        plt.colorbar()
        plt.title("alpha_vs_mask")
    ...
    print(psnrs.mean(), ssim.mean(), lpips.mean())
   ...
   # save the psnr result as the name of a dummy file
    file1 = open('%s/%06d/PSNR_%f_ssim_%f_lpips_%f' % (results_folder, iteration, psnrs.mean(), ssim.mean(), lpips.mean()), "a")
    file1.close()
    if save_checkpoint:
