...
def apply_edit(
    ...
    output_folder_final,
    mask_frames,
    evaluate_all_pixels=False,
    texture_edit_im1=None,
    texture_edit_im2=None,
    alpha_im1=None,
    alpha_im2=None,
    input_files="",
    frontend_save_name="./",
    ...
    imageio.imwrite(
        ...
        (
            np.concatenate((texture_edit_im1, alpha_im1[:, :, np.newaxis]), axis=2)
            * 255
        ).astype(np.uint8),
    )
    imageio.imwrite(
        "%s/user_provided_edited_atlas_background.png" % output_folder_final,
        (
            np.concatenate((texture_edit_im2, alpha_im2[:, :, np.newaxis]), axis=2)
            * 255
        ).astype(np.uint8),
    )
    ...
    miny = 0
    edge_size = 1
    _, texture_orig1 = get_high_res_texture(
        ReconstructResolution, 0, 1, 0, 1, model_F_atlas, device
    )
    ...
    _, texture_orig2 = get_high_res_texture(
        ReconstructResolution,
        minx2,
        minx2 + edge_size2,
        miny2,
        miny2 + edge_size2,
    ...
    edited_tex1 = (
        ...
        + torch.from_numpy(alpha_im1).unsqueeze(-1) * texture_edit_im1
    )

    edited_tex1_only_edit = torch.cat(
        (edited_tex1_only_edit, torch.from_numpy(alpha_im1).unsqueeze(-1)), dim=-1
    )
    edited_tex2_only_edit = torch.from_numpy(texture_edit_im2)
    edited_tex2 = (
        torch.from_numpy(1 - alpha_im2).unsqueeze(-1) * texture_orig2
        + torch.from_numpy(alpha_im2).unsqueeze(-1) * texture_edit_im2
    )

    edited_tex2_only_edit = torch.cat(
        (edited_tex2_only_edit, torch.from_numpy(alpha_im2).unsqueeze(-1)), dim=-1
    )
    ...
    with torch.no_grad():
        ...
        for f in tqdm(
            range(number_of_frames), "Reconstruct the Frames from Edited Atlas"
        ):
            ...
            for i in range(len(relisa)):
                ...
                rgb_only_edit, pointsx1, pointsy1, relevant1_only_edit = get_colors(
                    ReconstructResolution,
                    minx,
                    minx + edge_size,
                    miny,
                ...
                rgb_only_edit2, pointsx2, pointsy2, relevant2_only_edit = get_colors(
                    ReconstructResolution,
                    minx2,
                    minx2 + edge_size2,
                    miny2,
                ...
                only_mapped_texture[
                    relisa[i][relevant1_only_edit], reljsa[i][relevant1_only_edit], :, f
                ] = rgb_only_edit

                only_mapped_texture2[
                    relisa[i][relevant2_only_edit], reljsa[i][relevant2_only_edit], :, f
                ] = rgb_only_edit2
            # see details in Section 3.4 in the paper
            foreground_edit_cur = only_mapped_texture[
                :, :, :3, f
            ]  # denoted in the paper by c_{ef}
            foreground_edit_cur_alpha = only_mapped_texture[:, :, 3, f][
                :, :, np.newaxis
            ]  # denoted by \alpha_{ef}

            background_edit_cur = only_mapped_texture2[
                :, :, :3, f
            ]  # denoted in the paper by c_{eb}
            background_edit_cur_alpha = only_mapped_texture2[:, :, 3, f][
                :, :, np.newaxis
            ]  # denoted in the paper by \alpha_{eb}

            video_frame_cur = (
                video_frames[:, :, :, f].cpu().clone().numpy()
            )  # denoted in the paper by \bar{c}_{b}

            # Equation (15):
            video_frame_cur_edited1 = foreground_edit_cur * (
                foreground_edit_cur_alpha
            ) + video_frame_cur * (
                1 - foreground_edit_cur_alpha
            )  # \bar{c}_b
            video_frame_cur_edited2 = background_edit_cur * (
                background_edit_cur_alpha
            ) + video_frame_cur * (
                1 - background_edit_cur_alpha
            )  # \bar{c}_f
            ...
            if mask_of_new_obj:
                gray_fore = cv2.cvtColor(
                    (video_frame_cur_edited1 * 255).astype(np.uint8), cv2.COLOR_BGR2GRAY
                )
                cur_alpha = cur_alpha * (gray_fore[:, :, None] > 0).astype(np.float32)
            final_edited_frame_output = (
                video_frame_cur_edited1 * cur_alpha
                + (1 - cur_alpha) * video_frame_cur_edited2
            )
    ...
    mpimg.imsave(
        "%s/atlas_ori_foreground.png" % output_folder_final,
        (masks1[:, :, np.newaxis] * texture_orig1.numpy() * (255)).astype(np.uint8),
    )
    mpimg.imsave(
        "%s/atlas_ori_background.png" % output_folder_final,
        (masks2[:, :, np.newaxis] * texture_orig2.numpy() * (255)).astype(np.uint8),
    )
    writer_edit = imageio.get_writer(
        "%s/results_based_on_edited_atlas.mp4" % (output_folder_final), fps=10
    )
...
def Atlas2frames(
    ...
    if ori_edit_im1.shape[2] == 3:
        new_image = np.zeros(
            (ori_edit_im1.shape[0], ori_edit_im1.shape[1], 4), dtype=np.uint8
        )
