...
def load_input_data(resy, resx, maximum_number_of_frames, data_folder, use_mask_rcnn_bootstrapping, filter_optical_flow, vid_root):
    flow_dir = vid_root / 'flow'
    mask_dir = vid_root / 'mask'
    propainter_dir = vid_root / 'propainter' / 'images'

    input_files = sorted(list(data_folder.glob('*.jpg')) + list(data_folder.glob('*.png')) + list(data_folder.glob('*.JPG')))
    propainter_files = sorted(list(propainter_dir.glob('*.jpg')) + list(propainter_dir.glob('*.png'))+ list(propainter_dir.glob('*.JPG')))
    ...
    mask_files = sorted(list(mask_dir.glob('*.jpg')) + list(mask_dir.glob('*.png'))+ list(mask_dir.glob('*.JPG')))
    for i in range(number_of_frames):
        file1 = input_files[i]
        painter_file = propainter_files[i]
        im = np.array(Image.open(str(file1))).astype(np.float64) / 255.
        im_painter = np.array(Image.open(str(painter_file))).astype(np.float64) / 255.
        ...
        propainter_frames[:, :, :, i] = torch.from_numpy(cv2.resize(im_painter[:, :, :3], (resx, resy)))
    ...
    return optical_flows_mask, video_frames, propainter_frames, optical_flows_reverse_mask, mask_frames, video_frames_dx, video_frames_dy, optical_flows_reverse, optical_flows
...
def pre_train_mapping(model_F_mapping, frames_num, uv_mapping_scale, resx, resy, larger_dim, device, pretrain_iters=500):
    ...
    for i in range(pretrain_iters):
        ...
        for f in range(1):
            ...
            xyt = torch.cat((j_s, i_s, (f / (frames_num / 2.0) - 1) * torch.ones_like(i_s)), dim=1).to(device)  # [-1, 1]
            out = model_F_mapping(xyt)
            uv_temp_fore, uv_temp_back = out[..., :2], out[..., 2:4]

            model_F_mapping.zero_grad()
            loss_fore = (xyt[:, :2] * uv_mapping_scale - uv_temp_fore).norm(dim=1).mean()
            loss_back = (xyt[:, :2] * uv_mapping_scale - uv_temp_back).norm(dim=1).mean()
