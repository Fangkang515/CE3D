...
def get_ori_img_size(data_folder):
    paths = glob.glob(f'{data_folder}/*.png') + glob.glob(f'{data_folder}/*.jpg') + glob.glob(f'{data_folder}/*.JPG')
...
def getting_optical_flow(vid_frames_path):
    ...
    files = sorted(list(vid_path.glob("*.jpg")) + list(vid_path.glob("*.png")) + list(vid_path.glob("*.JPG")))
...
def main(config):
    ...
    if True:  # parse parameters
        ...
        alpha_bootstrapping_factor = config["alpha_bootstrapping_factor"]  # default=2000
        stop_bootstrapping_iteration = config["stop_bootstrapping_iteration"]  # default=10000
    ...
    vid_root = data_folder.parent / 'work_space' / 'atlas_space'   # .../scene/work_space/atlas_space
    ...
    optical_flows_mask, video_frames, propainter_frames, optical_flows_reverse_mask, mask_frames, video_frames_dx, video_frames_dy, optical_flows_reverse, optical_flows = load_input_data(resy, resx, maximum_number_of_frames, data_folder, True, True, vid_root)
    ...
    optimizer_all = optim.AdamW([{'params': list(model_F_mapping.parameters()), 'lr': 0.001},
    ...
    if not load_checkpoint:  # pretrain
        if pretrain_mapping1:
            model_F_mapping = pre_train_mapping(model_F_mapping, number_of_frames, uv_mapping_scale, resx=resx, resy=resy,
    ...
    for i in range(start_iteration, iters_num):
        ...
        rgb_propainter_background_GT = propainter_frames[jif_current[1, :], jif_current[0, :], :, jif_current[2, :]].squeeze(1).to(device)
        # the correct alpha according to the precomputed mask
        alpha_mask = mask_frames[jif_current[1, :], jif_current[0, :], jif_current[2, :]].squeeze(1).to(device).unsqueeze(-1)
        ...
        rgb_output1 = (model_F_atlas(uv_foreground1 * 0.25 + 0.75) + 1.0) * 0.5  # [-1,1]*0.25+0.75 = [0.5,1]; ([0,1]+1)*0.5=[0.5,1]
        rgb_output2 = (model_F_atlas(uv_foreground2 * 0.25 + 0.25) + 1.0) * 0.5  # [-1,1]*0.5+0.25 = [0,0.5];([-1,0]+1)*0.5=[0,0.5]
        ...
        if use_propainter_loss:
            rgb_loss_propainter = (torch.norm(rgb_output2 - rgb_propainter_background_GT, dim=1) ** 2).mean()
        else:
        ...
        # =========================  RGB gradient loss ========================= #
        if use_gradient_loss:  # XXX TODO default not use, since the trade-off between of performance and speed
            gradient_loss = get_gradient_loss(video_frames, video_frames_dx, video_frames_dy, jif_current,
                                              model_F_mapping, model_F_atlas,
                                              rgb_output_pred, device, resx, number_of_frames)
        else:
        ...
        rigidity_loss1 = get_rigidity_loss(
            ...
            uv_mapping_scale=uv_mapping_scale, is_foreground=True)
        rigidity_loss2 = get_rigidity_loss(
            ...
            uv_mapping_scale=uv_mapping_scale, is_foreground=False)
        if include_global_rigidity_loss and i <= stop_global_rigidity:
            ...
            global_rigidity_loss1, global_rigidity_loss2 = torch.tensor(0.), torch.tensor(0.)
        ...
        flow_loss1 = get_optical_flow_loss(
            ...
            number_of_frames, model_F_mapping, optical_flows, optical_flows_mask, uv_mapping_scale, device, use_alpha=True,
        ...
        flow_alpha_loss = get_optical_flow_alpha_loss(model_F_mapping,
        ...
        if i > stop_bootstrapping_iteration:
            alpha_bootstrapping_loss = torch.tensor(0.)
        else:
            alpha_bootstrapping_loss = torch.mean(-alpha_mask * torch.log(alpha) - (1 - alpha_mask) * torch.log(1 - alpha))

        loss = rigidity_coeff * (rigidity_loss1 + rigidity_loss2) + \
            ...
            gradient_loss * gradient_loss_coeff   # + rgb_loss2 * gradient_loss_coeff
        ...
        try:
            if use_optical_flow and i % print_every == 0:
                print("of_loss1:%.2f" % flow_loss1.detach(), "of_loss2:%.2f" % flow_loss2.detach())
        ...
        if i % print_every == 0:
            ...
            print(f'Iteration {i}  using time: {using_time:.2f}  lr_mapping: {lr_mapping_network:.5f}  lr_atlas: {lr_atlas_network:.5f}')
            logging.info('Iteration %d' % i)
            print("gradient_loss: %.3f" % gradient_loss.cpu().item())
            print("flow alpha loss: %.3f" % flow_alpha_loss.cpu().item(), "alpha_balancing_loss %.3f" % alpha_bootstrapping_loss.cpu().item())
            print("rgb_loss:%.3f" % rgb_loss.detach(), 'rgb_loss_negative %.3f' % rgb_loss_sparsity.detach())
            print("rigidity_loss1:%.3f" % rigidity_loss1.detach(), "rigidity_loss2:%.3f" % rigidity_loss2.detach())
            print("total_loss:%.3f" % loss.detach())
            print("alpha_mean:%.3f" % alpha.mean().detach(), "Big0.5_mean:%.3f" % alpha[alpha > 0.5].mean().detach(), "Low0.5_mean:%.3f" % alpha[alpha < 0.5].mean().detach())
        ...
        if (i % evaluate_every == 0 and i > start_iteration) or (i == iters_num-1) or (pretrain_mapping1 and pretrain_mapping2 and i == 1000):
            psnrs, ssim, lpips = evaluate_model(model_F_atlas, resx, resy, number_of_frames, model_F_mapping,
                           video_frames, results_folder, i, mask_frames, optimizer_all,
                           writer, derivative_amount, uv_mapping_scale,
                           optical_flows,
                           optical_flows_mask, device)
            ...
            with open(f'{metric_path}/{add_to_experiment_folder_name}.txt', 'a') as file:
                file.write(f'{scene_name} i:{i} psnr:{psnrs.mean():.2f} ssim:{ssim.mean():.2f} lpips:{lpips.mean():.2f} t:{using_time/60:.2f}min\n')
                if i == iters_num-1:
                    file.write('\n\n')
...
if __name__ == "__main__":
    ...
    for scn in scns[:]:
        begin_time = time()
        data_folder = f"Your/dataset_zoos/llff/{scn}/images_2"
