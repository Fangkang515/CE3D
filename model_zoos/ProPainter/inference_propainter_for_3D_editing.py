...
def inference_propainter(video_path, mask_path, save_path, args=None, use_half=False):
    device = get_device()
    ...
    save_root = os.path.join(save_path, 'images')
    ...
    # save each frame
    if args.save_frames:
        frame_names = sorted(os.listdir(args.video))
        assert len(frame_names) == video_length
        for idx in range(video_length):
            f = comp_frames[idx]
            f = cv2.resize(f, out_size, interpolation = cv2.INTER_CUBIC)
            f = cv2.cvtColor(f, cv2.COLOR_BGR2RGB)
            # img_save_root = os.path.join(save_root, 'frames', str(idx).zfill(4)+'.png')
            img_save_root = os.path.join(save_root, frame_names[idx])
    ...
    imageio.mimwrite(os.path.join(os.path.dirname(save_root), 'masked_video_show.mp4'), masked_frame_for_save, fps=fps, quality=7)
    imageio.mimwrite(os.path.join(os.path.dirname(save_root), 'inpaint_video_show.mp4'), comp_frames, fps=fps, quality=7)
...
def config_parser_ProPainter(exist_parser):
    # config for ProPainter
    parser = exist_parser
    parser.add_argument(
